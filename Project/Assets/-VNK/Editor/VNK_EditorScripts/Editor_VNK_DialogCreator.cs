...
public class Editor_VNK_DialogCreator : Editor {
    ...
    public override void OnInspectorGUI() {
		...
		showChangeBack = EditorGUILayout.Foldout(showChangeBack, "Change background on Next/Question");
		if(showChangeBack){
			EditorGUI.indentLevel = 1;
			EditorGUIUtility.labelWidth = 230.0f;
			myInspector.changeBackground = EditorGUILayout.Toggle("Want to change background?", myInspector.changeBackground);
			if(myInspector.changeBackground){
				EditorGUIUtility.labelWidth = 120.0f;
				myInspector.fadeChanging = EditorGUILayout.Toggle("Fade when change?", myInspector.fadeChanging);
				if(myInspector.dialogType == 0){
					myInspector.newBackground = (Sprite) EditorGUILayout.ObjectField("New background: ", myInspector.newBackground, typeof(Sprite), true);
				}
			}
			EditorGUI.indentLevel = 0;
		}
		...
		if(showPlayAudio){
			...
			myInspector.bgAmbienceClip = (AudioClip) EditorGUILayout.ObjectField("Ambience background: ", myInspector.bgAmbienceClip, typeof(AudioClip), true);
			myInspector.bgFxClip = (AudioClip) EditorGUILayout.ObjectField("Background SoundFx: ", myInspector.bgFxClip, typeof(AudioClip), true);
			myInspector.bgMusicClip = (AudioClip) EditorGUILayout.ObjectField("Background music: ", myInspector.bgMusicClip, typeof(AudioClip), true);
			if(myInspector.dialogType == 0){
		...
		if(myInspector.dialogType == 0){
			...
			myInspector.saveState = EditorGUILayout.Toggle("Autosave current state:", myInspector.saveState);
			if(myInspector.saveState){
				EditorGUI.indentLevel = 1;
				myInspector.saveCurrentLevel = EditorGUILayout.Toggle("Save current Level:", myInspector.saveCurrentLevel);
				if(myInspector.saveCurrentLevel){
					myInspector.levelName = EditorGUILayout.TextField("Level Name:", myInspector.levelName);
				}
				EditorGUI.indentLevel = 0;
			}
		...
		if(myInspector.dialogType == 1){
			...
			if(myInspector.changeBackground){
				myInspector.newBackground_question01 = (Sprite) EditorGUILayout.ObjectField("Question 01 to background: ", myInspector.newBackground_question01, typeof(Sprite), true);
			}
			...
			if(myInspector.changeBackground){
				myInspector.newBackground_question02 = (Sprite) EditorGUILayout.ObjectField("Question 02 to background: ", myInspector.newBackground_question02, typeof(Sprite), true);
			}
			...
			if(myInspector.changeBackground){
				myInspector.newBackground_question03 = (Sprite) EditorGUILayout.ObjectField("Question 03 to background: ", myInspector.newBackground_question03, typeof(Sprite), true);
			}
			...
			if(myInspector.changeBackground){
				myInspector.newBackground_question04 = (Sprite) EditorGUILayout.ObjectField("Question 04 to background: ", myInspector.newBackground_question04, typeof(Sprite), true);
			}
		}
		#endregion
